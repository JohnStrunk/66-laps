...
Then('the records should be ordered chronologically with the most recent at the top', async function (this: CustomWorld) {
  ...
  for (let i = 0; i < history.length - 1; i++) {
    expect(history[i].startTime >= history[i + 1].startTime, `Record ${i} (${history[i].startTime}) should be >= Record ${i+1} (${history[i+1].startTime})`).toBeTruthy();
  }
  ...
  expect(firstCardText?.includes(firstHistoryItem.event), `First card should show "${firstHistoryItem.event}", got "${firstCardText}"`).toBeTruthy();
});
